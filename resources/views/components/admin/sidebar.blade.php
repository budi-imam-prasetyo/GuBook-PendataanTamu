            ...
            <ul class="gap-2 flex flex-col pl-0 relative h-[86%]">
                ...
                <li class="relative">
                    ...
                    <ul id="dropdown-menu" class="hidden flex-col space-y-2 py-2 pl-14 transition-all">
                        <li>
                            <a href="/admin/laporan-tamu"
                                class="{{ request()->is('admin/laporan-tamu') ? 'rounded-lg bg-secondaryBlue/13 font-semibold text-primaryBlue' : 'ease' }} block text-sm px-4 py-2 text-slate-700 hover:bg-primaryBlue/5 hover:text-primaryBlue dark:text-white rounded-md select-none transition-all">Laporan
                                Tamu</a>
                        </li>
                        <li>
                            <a href="/admin/laporan-kurir"
                                class="{{ request()->is('admin/laporan-kurir') ? 'rounded-lg bg-secondaryBlue/13 font-semibold text-primaryBlue' : 'ease' }} block text-sm px-4 py-2 text-slate-700 hover:bg-primaryBlue/5 hover:text-primaryBlue dark:text-white rounded-md select-none transition-all">Laporan
                                Kurir</a>
                        </li>
                    </ul>
                </li>

                <x-admin.sidebar-btn src="{{ asset('assets/icons/map-marker.svg') }}"
                    href="/admin/kunjungan">kunjungan</x-admin.sidebar-btn>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (['/admin/laporan-tamu', '/admin/laporan-kurir'].includes(currentPath)) {
        toggleDropdown();
    }
});
</script>